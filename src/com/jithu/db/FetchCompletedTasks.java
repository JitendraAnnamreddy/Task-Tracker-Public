...
public class FetchCompletedTasks {
	public String fetchtasks(String username) {
		...
		if (con == null) {
			...
			String statement = "select * from public.\"taskTable\" where ((assignedby=? or assignedto=?) and status='Completed') order by taskid DESC;";
			try {
				...
				if (result == null) {
					...
					while (result.next()) {
						...
						// String assignedBy = result.getString(2);
						String color = "";
						...
						reqHTML += "<div class=\"card" + color + "\">\r\n" + "		<div class=\"card-body\">\r\n"
								...
								+ "						  					<label style=\"color:black\">Task Status"
								+ "" + "</label>\r\n"
